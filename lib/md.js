(function() {
  ...
  R_IGNORE_CHILDREN = /^(APPLET|AREA|AUDIO|BUTTON|CANVAS|DATALIST|EMBED|HEAD|INPUT|MENU|METER|NOFRAMES|NOSCRIPT|OBJECT|OPTGROUP|OPTION|PARAM|PROGRESS|RP|RT|RUBY|SCRIPT|SELECT|STYLE|TEXTAREA|TITLE|VIDEO)$/;
  ...
  HtmlParser = (function() {
    ...
    HtmlParser.prototype.tr = function() {
      var _this = this;
      this.output("|");
      this.tableColumns[++this.tableDepth] = [];
      return function() {
        var column, tableColumns, _i, _len;
        tableColumns = _this.tableColumns[_this.tableDepth];
        if (tableColumns.length) {
          _this.output("\n|");
          for (_i = 0, _len = tableColumns.length; _i < _len; _i++) {
            column = tableColumns[_i];
            switch (column) {
              case "right":
                _this.output("----:");
                break;
          ...
          delete _this.tableColumns[_this.tableDepth--];
        }
        return _this.output("\n");
      };
    };
    ...
    HtmlParser.prototype.process = function(ele) {
      var after, after1, after2, alignment, childNode, err, href, i, ignoreEmptyChild, level, skipChildren, src, suffix, summary, title, _base, _i, _len, _ref, _ref1;
      ...
      if (ele.nodeType === this.win.Node.ELEMENT_NODE) {
          ...
          if (R_IGNORE_CHILDREN.test(ele.tagName)) {
            ...
            switch (ele.tagName) {
              ...
              case 'IMG':
                ...
                if (ele.hasAttribute('ismap')){
                  this.output(ele.outerHTML.replace(/\s?style=""/, ""))
                } else {
              ...
              case 'MAP':
                ...
                break;
              case 'TH':
                alignment = ele.style ? ele.style.textAlign : "left";
                this.tableColumns[this.tableDepth].push(alignment);
                after = this.outputLater("|");
                break;
              case 'TD':
                after = this.outputLater("|");
        ...
        if (!skipChildren) {
          ...
          for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
            childNode = _ref1[_i];
            if (ignoreEmptyChild === false || false === /\s+/.test(childNode.nodeValue)) {
